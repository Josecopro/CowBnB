...
async function obtenerNDVI(coordenadas, diasAtras = 30) {
    ...
    if (JSON.stringify(coordenadas[0]) !==
  ...
  `;
    ...
    const response = await axios.post("https://sh.dataspace.copernicus.eu/api/v1/statistics", statBody, {
        headers: {
    ...
    if (intervalos.length === 0) {
        throw new Error("No hay imágenes Sentinel-2 disponibles para este período (posible cobertura de nubes)");
    }
